...
immnune_auto_step2 <- function(exposure_data,output_path='result_step2.csv',
                         multiprocess=FALSE,start_num=NA,end_num=NA,
  ...
  for (file_id in file_list){
    ...
    cat('(',z,'/',zz,') Analysing',' \'',outcome_name,'\'',' to \'',exposure_name,'\'',sep = '')
    # 判断是否已经做过
    ...
    while (TRUE) {
      tryCatch({
        exp_dat <- TwoSampleMR::extract_outcome_data(snps = exp_dat_clump$SNP,outcomes = file_id_gwas)
        break
